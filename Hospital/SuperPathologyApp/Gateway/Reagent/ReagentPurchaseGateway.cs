...
{
    class ReagentPurchaseGateway : DbConnection
    {
        ...
        internal string Save(PurchaseModel aMdl, string comeFrom)
        {
            try
            {
                ...
                {
                    aMdl.BillNo = GetInvoiceNo(9, _trans, ConLab);
                ...
                {
                    DeleteInsertLab("INSERT INTO tb_reagent_PURCHASE_DETAIL(MasterId, ItemId, Qty, UnitPrice, BQty, UnitTotal, VatPc, VatAmt, ComPc, ComAmt, Tp, ExpireDate)VALUES(" + aMdl.BillId + ", '" + model.Id + "', '" + model.Qty + "', '" + model.PurchasePrice + "', '" + model.BQty + "','" + model.UnitTotal + "', '" + model.VatPc + "', '" + model.Vat + "', '" + model.TaxPc + "','" + model.Tax + "','" + model.Tp + "','" + model.ExpireDate.ToString("yyyy-MM-dd") + "')", _trans, ConLab);
                    Hlp.InsertIntoStockLedgerReagent(aMdl.BillId, aMdl.BillNo, Hlp.GetServerDate(ConLab, _trans), model.Id, model.PurchasePrice, 0, model.Qty , 0, "Purchase", model.LotNo,model.ExpireDate,  _trans, ConLab);
                }
                Hlp.InsertIntoPurchaseLedgerReagent(aMdl.Supplier.Id, aMdl.BillNo, Hlp.GetServerDate(ConLab, _trans), aMdl.BillId, aMdl.NetAmount, aMdl.TotalLess, 0, aMdl.TotalPaid, Hlp.UserName, _trans, ConLab);
        ...
        internal DataTable GetInvoiceList(DateTime dateTime, string searchString, string comeFrom)
        {
            try
            {
                ...
                string query = @"SELECT a.BillNo,a.BillDate,a.ReceiptNo,b.Name AS SupplierName,a.PostedBy, a.NetAmt AS TotAmt FROM tb_reagent_PURCHASE_MASTER a INNER JOIN tb_REAGENT_SUPPLIER b ON a.SuppId=b.Id   " + cond + " Order by a.BillNo Desc";
                ConLab.Open();
