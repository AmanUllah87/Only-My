...
{
    public partial class IndoorDueCollUi : Form
    {
        



        public IndoorDueCollUi()
        {
            InitializeComponent();
        ...
        IndoorDueCollGateway _gt = new IndoorDueCollGateway();
            ...
            if (e.KeyChar == 13)
            {
                ...
                switch (textInfo)
                {
                    case "1":
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            billNoTextBox.Text = dt.Rows[0]["MasterId"].ToString();
                            billDateTextBox.Text = Convert.ToDateTime(dt.Rows[0]["ReleaseDate"].ToString()).ToString("dd-MMM-yyyy");
                            ptNameTextBox.Text = dt.Rows[0]["PatientName"].ToString();
                            ageTextBox.Text = _gt.FncReturnFielValueLab("tb_in_ADMISSION", "Id='" + gccode + "'", "PtAge");
                            int consId = Convert.ToInt32(_gt.FncReturnFielValueLab("tb_in_ADMISSION", "Id='" + gccode + "'", "UnderDrId"));
                            consDrNameTextBox.Text = _gt.FncReturnFielValueLab("tb_DOCTOR", "Id='" + consId + "'", "Name");
                            balanceTextBox.Text = dt.Rows[0]["Balance"].ToString();
                            contactNoTextBox.Text = _gt.FncReturnFielValueLab("tb_in_ADMISSION", "Id='" + gccode + "'", "ContactNo");
        ...
        private void saveAndPrintButton_Click_1(object sender, EventArgs e)
        {
            ...
            if (CheckSuccess())
            {
              ...
              //  aMdl.BillNo = billNoTextBox.Text;
                //aMdl.BillId = Convert.ToInt32(_gt.FncReturnFielValueLab("tb_IN_BILL_MASTER", "BillNo='" + aMdl.BillNo + "'", "Id"));
                aMdl.BillId =Convert.ToInt32(billNoTextBox.Text);
                ...
                aMdl.LessPc =Convert.ToDouble(Hlp.IsNumeric(discountPcTextBox.Text)?discountPcTextBox.Text:"0");
              ...
              //  aMdl.TotalAmt =Convert.ToDouble(totalAmtTextBox.Text);
                ...
                aMdl.TotalLessAmt = Convert.ToDouble(Hlp.IsNumeric(totDiscountTextBox.Text)?totDiscountTextBox.Text:"0");
                aMdl.CollAmt = Convert.ToDouble(Hlp.IsNumeric(paidAmtTextBox.Text)?paidAmtTextBox.Text:"0");
        ...
        {
            string query = "SELECT * FROM V_in_Due_Collection_List WHERE TrNo='"+ invNo +"'";
            var dt = new IndoorReportViewer("in_DueColl", query, "Invoice", "V_in_Due_Collection_List", "");
            dt.ShowDialog();
            //helpPanel.Visible = false;
        }
        ...
        private void dataGridView2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode==Keys.Delete)
            {
                if (textInfo=="enter")
                {
                    ...
                    {
                        string invNo = dataGridView2.CurrentRow.Cells[1].Value.ToString();
                        _gt.DeleteInsertLab(@"INSERT INTO  DEL_RECORD_OF_BILL_DELETE  (BillNo, BillDate, BillTime, RegId, PatientName, MobileNo, Address, Age, Sex, RefDrId, UnderDrId, TotalAmt, LessAmt, LessFrom, CollAmt, Remarks, PostedBy,ModuleName,Status,MasterId,PcName,IpAddress) SELECT a.TrNo, a.TrDate, a.TrTime, b.RegId, b.PtName, b.ContactNo, b.PtAddress, b.PtAge, b.PtSex, b.RefId, b.UnderDrId, a.TotalAmt,   a.LessAmt, a.LessFrom, a.TotalAmt , a.Remarks, '" + Hlp.UserName + "','In-DueColl','Pending',b.Id,'" + Environment.UserName + "','" + Hlp.IpAddress() + "' FROM tb_DUE_COLL a INNER JOIN V_Admission_List b ON a.AdmId=b.Id WHERE a.TrNo='" + invNo + "'");                        
        ...
        private void IndoorDueCollUi_Load(object sender, EventArgs e)
        {
            //  Hlp.GridColor(dataGridView1);
            dueCollDateTextBox.Text = Hlp.GetServerDate().ToString("dd-MMM-yyyy");
            // genderComboBox.SelectedIndex = 0;
