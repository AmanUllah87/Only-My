...
{
    public partial class ReportPrintMicroGrowthUi : Form
    {
        string _invNo = "";
        ...
        public ReportPrintMicroGrowthUi()
        {
            InitializeComponent();
        ...
        private void GetInvoiceDetailByInvNo()
        {
            ...
            var data = _gt.GetMasterData(masterId);
            ...
            var dtls = _gt.GetDetailsData(masterId,Hlp.GroupName);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            LabReportquery = "SELECT * FROM A_TMP_Lab_ReportView WHERE MasterId='" + masterId + "'  AND ReportFileName='" + _reportFileName + "' AND PrintBy='"+ Hlp.UserName +"' ";
        ...
        private void GetDataForInvoicePrint()
        {
          ...
          //  totalCountTextBox.Text = @"0";
            ...
            {
                masterId = Convert.ToInt32(_gt.FncReturnFielValueLab("tb_BILL_MASTER", "BillNo='" + _invNo + "'", "Id"));
                if (_gt.FnSeekRecordNewLab("V_Due_Invoice_List","MasterId="+ masterId +" AND Balance>0"))
                {
                    ...
                    {
                        double ratio = Convert.ToDouble(_gt.FncReturnFielValueLab("V_Due_Invoice_List", "MasterId=" + masterId + "", "SUM((Balance*100)/BillAmt)"));
                        double ratioInDb = Convert.ToDouble(_gt.FncReturnFielValueLab("tb_MASTER_INFO", "1=1", "ReportPrintDuePercentage"));
                        if (ratio > ratioInDb)
                        {
                            label6.Text = @"Due:" + _gt.FncReturnFielValueLab("V_Due_Invoice_List", "MasterId=" + masterId + "", "Balance");
        ...
        private void Save(string invNo)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dataGridView2.Rows.Count; i++)
                {
                    string defaultResult = dataGridView2.Rows[i].Cells[1].Value == null ? "" : dataGridView2.Rows[i].Cells[1].Value.ToString();
                    if (defaultResult.Trim() != "")
                    {
                        mdl.Add(new TestParamModel()
                        {
                            MachineParam = dataGridView2.Rows[i].Cells[0].Value.ToString(),
                            ReportParam = dataGridView2.Rows[i].Cells[0].Value.ToString(),
                            DefaultResult = defaultResult,
                            TestchartId = Convert.ToInt32(dataGridView2.Rows[i].Cells[2].Value),
                            ReportNo =  dataGridView2.Rows[i].Cells[3].Value == null ? "" : dataGridView2.Rows[i].Cells[3].Value.ToString(),
                            Comment=commentsRichTextBox.Text,
                            NormalRange = headerTextBox.Text,
                            ReportingGroup = "",
                ...
                if (_isClickSaveButton == false)
                {
                    foreach (var model in mdl)
                    {
                        ...
                        {
                            _gt.DeleteInsertLab("Update tb_BILL_DETAIL SET PrintStatus=1 WHERE MasterId='" + masterId + "'  AND TestId='" + model.TestchartId + "'");
                ...
                else
                {
                    foreach (var model in mdl)
                    {
                        if (model.DefaultResult.Length > 0)
                        {
                            _gt.DeleteInsertLab("Update tb_BILL_DETAIL SET PrintStatus=2 WHERE MasterId='" + masterId + "'  AND TestId='" + model.TestchartId + "' AND PrintStatus<>1");
                        }
                    }
                }
        ...
        private void saveAndPrintButton_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView2.Rows.Count; i++)
            {
                ...
                if (result.Trim() != "")
                {
                    ...
                    string tstName = dataGridView2.Rows[i].Cells[4].Value.ToString();
                  


                    if (testNameInGrid != tstName)
                    {
                        testNameInGrid = dataGridView2.Rows[i].Cells[4].Value.ToString();
                        if (testNameForRpt == "")
                        {
                            testNameForRpt = dataGridView2.Rows[i].Cells[4].Value.ToString();
                        }
                        else
                        {
                            testNameForRpt = testNameForRpt + "," + dataGridView2.Rows[i].Cells[4].Value.ToString();
                        }
                    }
            ...
            if (_isSaved)
            {
                ...
                LabReportquery = "SELECT '" + _testNameForView + "' AS TestName,'" + Hlp.UserName + "' AS PrintBy,'"+ reportHeader +"' AS ReportHeader,  * FROM V_REPORT WHERE MasterId=" + masterId + " " + testCond + "  AND IsPrint=1 ";
        ...
        private void dataGridView4_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView4.CurrentRow != null)
            {
                ...
                LabReportquery = "SELECT " + _testNameForView + " AS TestName, * FROM VW_GET_LAB_REPORT_VIEW WHERE MasterId='" + masterId + "' AND IsPrint=1   ";
            }
        ...
        {
            dataGridView2.Rows.Clear();
            var data = _gt.GetDetailsData(masterId,Hlp.GroupName).Where(n => n.ReportFileName == rptFileName);
            if (testCode!="")
            {
            ...
            {
                GetParameterDataByCode(masterId,Convert.ToInt32(item.TestCode));
            }
           // CalculateDifferentialCount();
        }
        ...
        {
           // totalCountTextBox.Text = "0";
            var data = _gt.GetDataFromMicrobiologyGrowthParameterDefinitionByTestCode();
            ...
            {
                var savedData = _gt.GetDataFromMachineDataDtls(mstId, testChartId, mdl.DrugName);
                ...
                dataGridView2.Rows.Add(mdl.DrugName, mdl.DefaultResult, testChartId, mdl.ReportNo, testName);

                string richtext = _gt.FncReturnFielValueLab("tb_REPORT_MASTER", "MasterId=" + masterId + "  AND ReportNo='" + mdl.ReportNo + "' ", "Comment");
                if (richtext != "")
                {
        ...
        private void dataGridView2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Delete)
            {
                ...
                _gt.DeleteInsertLab("UPDATE tb_REPORT_DETAILS SET IsPrint=0 WHERE  MasterId=" + masterId + " AND MachineParam='" + parameter + "'  ");
        ...
        private void ReportPrintMicroGrowthUi_Load(object sender, EventArgs e)
        {
            ...
            int invNoDigit = Convert.ToString(_gt.FncReturnFielValueLab("tb_BILL_MASTER", "1=1 Order by BillNo Desc", "TOP 1 BillNo")).Length;
            string firstTwoDigit = "";
            if (invNoDigit == 10)
            {
                firstTwoDigit = Convert.ToString(_gt.FncReturnFielValueLab("tb_BILL_MASTER", "1=1 Order by BillNo Desc", "TOP 1 BillNo")).Substring(0, 2);
            }
